...
The daemon SHALL read all configuration from environment variables. Required variables: `LISTEN_ADDR`, `DB_PATH`. Optional integration variables: `JWT_SIGNING_KEY`, `SLURM_API_URL`, `SLURM_JWT_TOKEN`, `SLURM_API_USER`, `SLURM_ADMIN_USER`, `SLURM_ADMIN_JWT_TOKEN`, `OS_AUTH_URL`, `OS_PROJECT_NAME`, `OS_USERNAME`, `OS_PASSWORD`, `AMQP_URL`, `SLURM_SIF_PATH`, `SLURM_SIF_FILE`, `SLURM_CLOUD_PARTITION`, `SSH_USER`, `SSH_PORT`, `SSH_OPTIONS`, `SSH_PRIVATE_KEY_PATH`, `SSH_POLL_INTERVAL`, and `SSH_POLL_TIMEOUT`.
