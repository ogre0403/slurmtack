...
The client SHALL submit a placeholder job to slurmrestd via `POST /slurm/v0.0.40/job/submit` using the effective workload identity headers (`X-SLURM-USER-NAME`, `X-SLURM-USER-TOKEN`) for that execution. It MUST request exclusive node allocation, include the specified constraint, partition, and account when provided, set `current_working_directory` to `/home/<workload-user>`, set `standard_output` and `standard_error` to files under `/home/<workload-user>`, resolve the placeholder SIF image as `/home/<workload-user>/<PLACEHOLDER_SIF_PATH>/<effective-placeholder-sif-file>`, export `SLURM_API_USER` and `SLURM_JWT_TOKEN` that match the effective workload identity, and return the allocated job ID on success. The effective placeholder SIF filename MUST come from the execution-scoped persisted filename, with a daemon-default fallback only for migrated executions that predate the new persisted field.

#### Scenario: Successful job submission with account and default SIF filename

- **WHEN** SubmitPlaceholderJob is called with a valid execution ID, partition `gpu-maint`, account `proj-123`, workload user `alice`, daemon `PLACEHOLDER_SIF_PATH=slurmtack/build/output`, and effective placeholder SIF filename `placeholder-agent.sif`
...
- **WHEN** SubmitPlaceholderJob is called with a valid execution ID, workload user `alice`, daemon `PLACEHOLDER_SIF_PATH=slurmtack/build/output`, and effective placeholder SIF filename `placeholder-agent-debug.sif`
