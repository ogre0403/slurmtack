...
Define the standalone host-side shell scripts that reconfigure and verify GPU passthrough state on a target node, so the orchestrator can stage and execute them over SSH around a reboot.
## Requirements
