...
The system SHALL accept a switch request via `POST /v1/switches` and return a 202 response with an execution ID and status URL. The request body MUST include `direction` and `requested_by`. It MAY include `node_name` (required for openstack_to_slurm) and `slurm_constraint` (for slurm_to_openstack).

#### Scenario: Successful slurm_to_openstack request
